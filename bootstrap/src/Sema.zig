    ...
    const ty = switch (ast.tag(node)) {
        ...
        .identifier => blk: {
            ...
            break :blk switch (sym) {
                ...
                .builtin_compiler_write_file, .builtin_write_entire_file, .builtin_make_directory_if_it_does_not_exist, .builtin_delete_directory, .builtin_file_exists, .builtin_file_set_position, .builtin_file_write, .builtin_file_read => Type.boolType(),
                .builtin_get_command_line_arguments, .builtin_get_cpu_info, .builtin_file_open, .builtin_string_builder_type, .builtin_split => Type.init(InternPool.well_known.any_type),
