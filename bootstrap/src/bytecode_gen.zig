...
const GenContext = struct {
    ...
    fn tryEmitExpandProcCall(ctx: *GenContext, expr: NodeIndex, diag: Diagnostic) !bool {
        ...
        for (params, 0..) |param_idx, i| {
            ...
            if (std.mem.eql(u8, param_type_text, "Code")) {
                continue;
            }
    ...
    fn exportVisibleValueNames(ctx: *GenContext, child: *GenContext, diag: Diagnostic) !void {
        ...
        while (it.next()) |entry| {
            ...
            const name = ctx.ast.tokenSlice(ctx.ast.mainToken(decl));
...
fn structSizeByName(ctx: *GenContext, name: []const u8, diag: Diagnostic) anyerror!?u64 {
    const ast = ctx.ast;
    const sym = ctx.resolved.lookup(name) orelse {
...
fn structFieldSizeByName(ctx: *GenContext, type_name: []const u8, field_name: []const u8, diag: Diagnostic) !?u64 {
    const ast = ctx.ast;
    const sym = ctx.resolved.lookup(type_name) orelse {
